...
class App(base.App):
    ...
    #         if indent == len(line): continue
    #         # check minimum
    #         mini = indent if mini == None else min(mini, indent)
    #     # all lines are empty
    #     if mini == None: return False
    #     # get next tab number
    #     nextTab = mini//tabSize+1
    #     # compute spaces
    #     spaces = tabSize*nextTab - mini
    #     # insert new spaces in each line
    #     for r in range(r1, r2+1):
    #         # build new line
    #         newline = spaces*' ' + self.chrBuf[r]
    #         # update
    #         self.chrBuf[r] = newline
    #     # shift cursor
    #     self.shiftToColumn(x+spaces)
    #     # shift block selection
    #     self.blockBegin = c1+spaces, r1
    #     self.blockEnd = c2+spaces, r2
    #     return True

    # def shiftTabLeft(self):
    #     tabSize = 4
    #     # get state
    #     X, Y = self.scroll
    #     x, y = self.cursor
    ...
    #     left, right = line[:x+X], line[x+X:]
    #     # get indentation
    #     nonspaces = len(left)-len(line.lstrip())
    #     # nop
    #     if nonspaces > 0: return False
    #     # get next tab number
    #     previousTab = (len(left)-1)//tabSize
    #     # compute spaces
    #     spaces = (x+X)-previousTab*tabSize
    #     # insert new spaces
    #     self.chrBuf[y+Y] =
    #     # adjust cursor position
    #     self.shiftToColumn(x+Y-spaces)
    ...
    #     previousTab = (mini-1)//tabSize
    #     # compute spaces to delete
    #     spaces = mini - previousTab*tabSize
    #     # delete spaces in each lines
    #     for r in range(r1, r2+1):
    #         self.chrBuf[r] = self.chrBuf[r][spaces:]
    #     # shift cursor
    #     self.shiftToColumn(max(0, x-spaces))
    #     # adjust block selection
    #     self.blockBegin = max(0, c1-spaces), r1
    #     self.blockEnd = max(0, c2-spaces), r2
    #     return True
